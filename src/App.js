...
/* ─── GLOBAL STYLES (self-contained, no external CSS needed) ── */
const CSS = `
  @import url('https://fonts.googleapis.com/css2?family=Nunito:wght@400;500;600;700;800;900&display=swap');
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  html, body, #root { height: 100%; font-family: 'Nunito', system-ui, sans-serif; -webkit-font-smoothing: antialiased; }
  ...
  .card-hover:hover { transform: translateY(-3px) !important; box-shadow: 0 10px 36px rgba(124,58,237,0.16) !important; border-color: #C4B5FD !important; }
  .quick-btn:hover { transform: scale(1.04) !important; }
`
function GlobalStyles() {
  useEffect(() => {
    if (document.getElementById('nume-styles')) return
    const style = document.createElement('style')
    style.id = 'nume-styles'
    style.textContent = CSS
    document.head.appendChild(style)
    return () => { const el = document.getElementById('nume-styles'); if (el) el.remove() }
  }, [])
  return null
}
...
const IC = {
  ...
  Learn: ({ s=20, c='currentColor' }) => (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
      <path d="M4 19.5A2.5 2.5 0 016.5 17H20"/>
      <path d="M6.5 2H20v20H6.5A2.5 2.5 0 014 19.5v-15A2.5 2.5 0 016.5 2z"/>
    </svg>
  ),
  Quiz: ({ s=20, c='currentColor' }) => (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
      <circle cx="12" cy="12" r="10"/>
      <path d="M9.09 9a3 3 0 015.83 1c0 2-3 3-3 3"/>
      <circle cx="12" cy="17" r=".5" fill={c}/>
    </svg>
  ),
  Dict: ({ s=20, c='currentColor' }) => (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
      <path d="M12 20h9"/>
      <path d="M16.5 3.5a2.121 2.121 0 013 3L7 19l-4 1 1-4L16.5 3.5z"/>
    </svg>
  ),
  Voice: ({ s=20, c='currentColor' }) => (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
  ...
  Heart: ({ s=16, c='currentColor', filled=false }) => (
    <svg width={s} height={s} viewBox="0 0 24 24" fill={filled ? '#FF4D6D' : 'none'} stroke={filled ? '#FF4D6D' : c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
...
const NAV = [
  ...
  { id: 'learn',      Icon: IC.Learn, label: 'Learn'      },
  { id: 'quiz',       Icon: IC.Quiz,  label: 'Quiz'       },
  { id: 'dictionary', Icon: IC.Dict,  label: 'Dictionary' },
  { id: 'voice',      Icon: IC.Voice, label: 'Speak'      },
  { id: 'progress',   Icon: IC.Me,    label: 'Me'         },
]
...
function tickStreak(uid, d) {
  const data = getStreakData(uid, d)
  ...
  saveStreakData(uid, d, { lastDate: today, count })
  return count
}
...
async function speakEL(phonetic, key) {
  const ck = 'el_' + phonetic
  let blob = SPEECH_CACHE.get(ck)
  if (!blob) {
    const url = CORS_PROXY + encodeURIComponent('https://api.elevenlabs.io/v1/text-to-speech/' + EL_VOICE + '/stream')
    const res = await fetch(url, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'xi-api-key': key },
      body: JSON.stringify({ text: phonetic, model_id: EL_MODEL, voice_settings: { stability: 0.35, similarity_boost: 0.9, style: 0.55, use_speaker_boost: true } }),
    })
    if (!res.ok) throw new Error(res.status + ': ' + (await res.text().catch(() => res.statusText)).slice(0, 120))
    blob = await res.blob()
    SPEECH_CACHE.set(ck, blob)
  }
...
async function speak(word, apiKey) {
  stopAudio()
  const w = typeof word === 'string' ? { roman: word } : word
  const phonetic = ((w.roman || w.ijaw) + '').replace(/-/g, ' ').replace(/\//g, '').trim()
  if (apiKey) { try { await speakEL(phonetic, apiKey); return } catch (e) { console.warn('ElevenLabs:', e.message) } }
...
function MockBanner({ message, style }) {
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 9, background: '#FFD16622', border: '1.5px dashed #FFD166', borderRadius: 14, padding: '10px 14px', fontSize: 12.5, color: '#7A5C00', fontWeight: 600, lineHeight: 1.5, ...style }}>
...
function Card({ children, style, onClick, hover }) {
  ...
  return (
    <div onClick={onClick}
      ...
      style={{ background: B.white, borderRadius: 20, padding: 18, boxShadow: h ? '0 10px 36px rgba(124,58,237,0.14)' : '0 2px 12px rgba(26,16,51,0.07)', border: `2px solid ${h ? B.purpleLight : B.border}`, transition: 'all 0.22s cubic-bezier(0.34,1.56,0.64,1)', cursor: onClick ? 'pointer' : 'default', ...(style || {}) }}>
      {children}
    </div>
  )
}

function Btn({ children, onClick, bg, color, style, disabled, outline }) {
  const [h, setH] = useState(false)
  const bgColor = bg || (outline ? 'transparent' : disabled ? B.border : B.purple)
  const fgColor = color || (outline ? B.purple : '#fff')
  return (
    <button onClick={disabled ? undefined : onClick}
      onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}
      style={{ background: h && !disabled ? (outline ? `${B.purple}10` : B.purpleMid) : bgColor, color: disabled ? B.inkFaint : fgColor, border: outline ? `2px solid ${B.purple}` : 'none', borderRadius: 14, padding: '12px 20px', fontSize: 14.5, fontWeight: 800, cursor: disabled ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, transition: 'all 0.18s cubic-bezier(0.34,1.56,0.64,1)', letterSpacing: 0.2, boxShadow: (!outline && !disabled) ? `0 4px 16px ${B.purple}35` : 'none', transform: h && !disabled ? 'translateY(-1px)' : 'none', ...(style || {}) }}>
      {children}
    </button>
  )
}
...
function Badge({ children, color, bg }) {
  ...
  return <span style={{ background: bg || `${c}18`, color: c, border: `1.5px solid ${c}30`, borderRadius: 99, padding: '3px 11px', fontSize: 11.5, fontWeight: 800, whiteSpace: 'nowrap', letterSpacing: 0.2 }}>{children}</span>
}

function PBar({ value, max, color, h = 8, style }) {
  const c = color || B.purple
...
function HeartsRow({ count, max = 3 }) {
  return (
    <div style={{ display: 'flex', gap: 3 }}>
      {Array.from({ length: max }).map((_, i) => <IC.Heart key={i} s={17} c={i < count ? '#FF4D6D' : B.border} filled={i < count} />)}
    </div>
  )
}
...
function Bird({ size = 40, bob, style }) {
  return (
    <svg width={size} height={size} viewBox="0 0 200 200" style={{ ...(style || {}), ...(bob ? { animation: 'birdBob 2.2s ease-in-out infinite' } : {}) }}>
...
function AchievementModal({ achievement, onClose }) {
  ...
  function handleShare() {
    const text = `I just earned "${achievement.title}" on Nume — Ijaw Voice! ${achievement.icon} ${achievement.desc}.`
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(26,16,51,0.75)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, backdropFilter: 'blur(10px)' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: B.white, borderRadius: 28, padding: '40px 28px', maxWidth: 340, width: '100%', textAlign: 'center', position: 'relative', overflow: 'hidden', animation: visible ? 'pop 0.55s cubic-bezier(0.34,1.56,0.64,1) forwards' : 'none', opacity: visible ? 1 : 0, boxShadow: `0 32px 80px ${achievement.color}50` }}>
        ...
        {[0, 1, 2, 3, 4].map(i => (
          <div key={i} style={{ position: 'absolute', top: `${15 + i * 6}%`, left: `${8 + i * 20}%`, fontSize: 14, animation: `starPop 0.8s ${i * 0.12}s both` }}>⭐</div>
        ))}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          <button onClick={handleShare} style={{ background: B.purple, border: 'none', borderRadius: 14, padding: 13, color: '#fff', fontWeight: 800, fontSize: 14, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
            <IC.Share s={14} c="#fff" />Share
          </button>
          <button onClick={onClose} style={{ background: B.purplePale, border: `2px solid ${B.purpleLight}`, borderRadius: 14, padding: 13, color: B.purple, fontWeight: 800, fontSize: 14, cursor: 'pointer' }}>Continue</button>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div style={{ position: 'absolute', top: 'calc(100% + 10px)', right: 0, zIndex: 500, background: B.white, borderRadius: 22, padding: 20, width: 300, boxShadow: '0 16px 48px rgba(26,16,51,0.2)', border: `2px solid ${B.border}`, animation: 'fadeDown 0.18s ease' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
        ...
        <button onClick={onClose} style={{ background: B.surface, border: 'none', borderRadius: 50, width: 28, height: 28, cursor: 'pointer', color: B.inkSoft, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15 }}>×</button>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 3, marginBottom: 5 }}>
        {DAY_LABELS.map((d, i) => <div key={i} style={{ textAlign: 'center', fontSize: 9, fontWeight: 800, color: B.inkFaint }}>{d}</div>)}
      </div>
      {weeks.map((week, wi) => (
...
function SpeechModal({ apiKey, onSave, onClose }) {
  const [val, setVal] = useState(apiKey || '')
  const [testing, setTesting] = useState(false)
  const [result, setResult] = useState(null)
  async function testKey() {
    setTesting(true); setResult(null)
    try { await speakEL('Seridou', val); setResult('ok') } catch (e) { setResult('err:' + e.message) }
    setTesting(false)
  }
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(26,16,51,0.75)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, backdropFilter: 'blur(10px)' }} onClick={onClose}>
      <div style={{ background: B.white, borderRadius: 26, padding: 28, maxWidth: 420, width: '100%', boxShadow: '0 32px 80px rgba(124,58,237,0.3)' }} onClick={e => e.stopPropagation()}>
        ...
        <input value={val} onChange={e => setVal(e.target.value)} placeholder="sk-..." type="password" style={{ width: '100%', padding: '13px 16px', borderRadius: 14, border: `2px solid ${B.border}`, fontSize: 14, outline: 'none', boxSizing: 'border-box', marginBottom: 10, color: B.ink, background: B.offWhite }} />
        {result && <p style={{ fontSize: 13, margin: '0 0 10px', fontWeight: 700, color: result === 'ok' ? B.mint : B.coral }}>{result === 'ok' ? '✅ Voice working!' : result.replace('err:', '')}</p>}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
          <button onClick={testKey} disabled={!val || testing} style={{ padding: 12, borderRadius: 14, border: `2px solid ${B.purple}`, background: 'transparent', color: B.purple, cursor: (!val || testing) ? 'not-allowed' : 'pointer', fontWeight: 800, fontSize: 14 }}>{testing ? 'Testing…' : 'Test Voice'}</button>
          <button onClick={() => { onSave(val); onClose() }} style={{ padding: 12, borderRadius: 14, border: 'none', background: B.purple, color: '#fff', cursor: 'pointer', fontWeight: 800, fontSize: 14 }}>Save &amp; Close</button>
        </div>
        {val && <button onClick={() => { onSave(''); onClose() }} style={{ width: '100%', padding: 10, borderRadius: 14, border: `2px solid ${B.border}`, background: 'transparent', color: B.inkFaint, cursor: 'pointer', fontSize: 13 }}>Remove key</button>}
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(26,16,51,0.75)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, backdropFilter: 'blur(10px)' }} onClick={onClose}>
      <div style={{ background: B.white, borderRadius: 26, padding: 28, maxWidth: 420, width: '100%', boxShadow: '0 32px 80px rgba(124,58,237,0.3)' }} onClick={e => e.stopPropagation()}>
        ...
        {DIALECTS.map(d => {
          ...
          return (
            <div key={d} onClick={() => onSelect(d)} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 16px', borderRadius: 18, marginBottom: 10, cursor: 'pointer', background: d === dialect ? B.purplePale : B.offWhite, border: `2px solid ${d === dialect ? B.purple : B.border}`, transition: 'all 0.2s' }}>
        ...
        <button onClick={onClose} style={{ width: '100%', marginTop: 8, padding: 13, borderRadius: 14, border: `2px solid ${B.border}`, background: 'transparent', cursor: 'pointer', fontWeight: 800, color: B.inkSoft, fontSize: 14 }}>Cancel</button>
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{ width: col ? 68 : 230, minHeight: '100vh', background: `linear-gradient(180deg, ${B.ink} 0%, #0D0820 100%)`, display: 'flex', flexDirection: 'column', transition: 'width 0.26s cubic-bezier(0.34,1.56,0.64,1)', flexShrink: 0, position: 'sticky', top: 0, height: '100vh', borderRight: '1px solid rgba(255,255,255,0.07)' }}>
      <div style={{ padding: '20px 14px 16px', borderBottom: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', gap: 10 }}>
        <Bird size={32} />
        {!col && <span style={{ color: '#fff', fontSize: 22, fontWeight: 900, letterSpacing: -0.5 }}>Nume</span>}
      </div>
      {!col && user && (
      ...
      {!col && (
        <div style={{ padding: '10px 14px', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
          ...
          <button onClick={onSwitch} style={{ background: 'rgba(255,255,255,0.08)', border: '2px solid rgba(255,255,255,0.12)', borderRadius: 14, padding: '8px 12px', color: 'rgba(255,255,255,0.85)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 9, width: '100%', fontSize: 13.5, fontWeight: 800 }}>
            <span style={{ fontSize: 18 }}>{DIALECT_INFO[dialect].flag}</span>
            <span style={{ flex: 1, textAlign: 'left' }}>{dialect}</span>
            <IC.Edit s={13} c="rgba(255,255,255,0.35)" />
          </button>
        </div>
      )}
      <nav style={{ flex: 1, padding: '10px 8px' }}>
        {NAV.map(n => {
          const active = screen === n.id; const NavIcon = n.Icon
          return (
            <button key={n.id} onClick={() => setScreen(n.id)} title={col ? n.label : ''} className="nav-btn"
              style={{ display: 'flex', alignItems: 'center', gap: 11, width: '100%', padding: '11px 12px', borderRadius: 14, marginBottom: 3, background: active ? 'rgba(124,58,237,0.35)' : 'transparent', border: `2px solid ${active ? 'rgba(124,58,237,0.5)' : 'transparent'}`, color: active ? '#fff' : 'rgba(255,255,255,0.45)', cursor: 'pointer', fontSize: 14, fontWeight: active ? 800 : 500, transition: 'all 0.18s' }}>
              <NavIcon s={19} c={active ? '#fff' : 'rgba(255,255,255,0.45)'} />
              {!col && <span>{n.label}</span>}
              {!col && active && <div style={{ marginLeft: 'auto', width: 6, height: 6, borderRadius: 99, background: B.yellow }} />}
            </button>
          )
        })}
      </nav>
      {!col && (
        <button onClick={onLogout} style={{ background: 'transparent', border: 'none', color: 'rgba(255,120,120,0.7)', padding: '13px 16px', cursor: 'pointer', fontSize: 13.5, fontWeight: 700, textAlign: 'left', display: 'flex', alignItems: 'center', gap: 9, borderTop: '1px solid rgba(255,255,255,0.07)' }}>
          <IC.Logout s={17} c="rgba(255,120,120,0.7)" />Sign Out
        </button>
      )}
      <button onClick={() => setCol(!col)} style={{ background: 'transparent', border: 'none', color: 'rgba(255,255,255,0.25)', padding: '13px 14px', cursor: 'pointer', fontSize: 13, borderTop: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', gap: 8, justifyContent: col ? 'center' : 'flex-start' }}>
        {col ? <IC.Expand s={17} c="rgba(255,255,255,0.3)" /> : <><IC.Collapse s={17} c="rgba(255,255,255,0.3)" /><span style={{ fontSize: 12 }}>Collapse</span></>}
      </button>
    </div>
  )
}
  ...
  return (
    <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 200, background: B.white, borderTop: `2px solid ${B.border}`, display: 'flex', justifyContent: 'space-around', padding: '8px 0 max(10px,env(safe-area-inset-bottom))' }}>
      {NAV.map(n => {
        const active = screen === n.id; const NavIcon = n.Icon
        return (
          <button key={n.id} onClick={() => setScreen(n.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '4px 0', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, flex: 1, color: active ? B.purple : B.inkFaint, transition: 'all 0.15s' }}>
            <div style={{ width: 34, height: 34, borderRadius: 12, background: active ? B.purplePale : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.18s' }}>
              <NavIcon s={20} c={active ? B.purple : B.inkFaint} />
            </div>
            <span style={{ fontSize: 9, fontWeight: active ? 900 : 600, letterSpacing: 0.3 }}>{n.label}</span>
          </button>
        )
      })}
    </div>
...
function TopBar({ title, onBack, dialect, onSwitch, right }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', padding: '13px 16px 11px', background: B.white, borderBottom: `2px solid ${B.border}`, position: 'sticky', top: 0, zIndex: 100 }}>
      {onBack && (
        <button onClick={onBack} style={{ background: B.purplePale, border: `2px solid ${B.border}`, borderRadius: 12, width: 38, height: 38, cursor: 'pointer', color: B.purple, marginRight: 10, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <IC.ArrowLeft s={17} c={B.purple} />
        </button>
      )}
      <span style={{ fontSize: 17, fontWeight: 900, color: B.ink, flex: 1 }}>{title}</span>
      {dialect && (
        <button onClick={onSwitch} style={{ background: B.purplePale, border: `2px solid ${B.border}`, borderRadius: 20, padding: '6px 13px', fontSize: 13, fontWeight: 800, cursor: 'pointer', color: B.purple, marginRight: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
...
function AuthScreen({ onLogin }) {
  ...
  function handleLogin() {
    ...
    setTimeout(() => {
      const db = getUsersDB()
      const found = Object.values(db).find(u => u.email.toLowerCase() === form.email.toLowerCase() && u.password === form.password)
      if (found) onLogin(found)
      else { setErr('Invalid email or password. Try amina@example.com / password123'); setBusy(false) }
    }, 900)
  }
  function handleSignup() {
    ...
    const db = getUsersDB()
    if (Object.values(db).find(u => u.email.toLowerCase() === form.email.toLowerCase())) { setErr('Account already exists. Sign in instead.'); return }
    setBusy(true)
    const emojis = ['👩🏾', '👨🏿', '👩🏿', '👦🏾', '👧🏿', '🧑🏾']
    setTimeout(() => {
      const nu = { id: 'u' + Date.now(), name: form.name, email: form.email, password: form.password, joinedDate: new Date().toLocaleDateString('en-GB', { month: 'short', year: 'numeric' }), avatar: emojis[Math.floor(Math.random() * emojis.length)] }
      saveUserToDB(nu); onLogin(nu)
    }, 900)
  }
  const inputSt = { width: '100%', padding: '13px 16px', borderRadius: 14, border: '2px solid rgba(255,255,255,0.15)', background: 'rgba(255,255,255,0.1)', color: '#fff', fontSize: 14.5, outline: 'none', boxSizing: 'border-box' }
    ...
    <div style={{ minHeight: '100vh', background: `linear-gradient(150deg, ${B.ink} 0%, #2D1050 50%, #0A1840 100%)`, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ maxWidth: 440, width: '100%', textAlign: 'center', position: 'relative', zIndex: 1, animation: 'fadeUp 0.6s ease' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 32 }}>
          <Btn onClick={() => setMode('signup')} bg={`linear-gradient(135deg, ${B.purple}, ${B.purpleMid})`} style={{ width: '100%', fontSize: 16, padding: '15px 20px' }}>Create Free Account</Btn>
          <Btn onClick={() => setMode('login')} outline style={{ width: '100%', fontSize: 16, padding: '15px 20px', color: 'rgba(255,255,255,0.8)', borderColor: 'rgba(255,255,255,0.3)' }}>Sign In</Btn>
        </div>
        <div style={{ background: 'rgba(255,255,255,0.06)', borderRadius: 20, padding: '18px 18px', border: '2px solid rgba(255,255,255,0.08)' }}>
          ...
          {MOCK_USERS.map(u => (
            <button key={u.id} onClick={() => { setForm({ email: u.email, password: u.password, name: u.name }); setMode('login') }} style={{ display: 'flex', alignItems: 'center', gap: 12, width: '100%', background: 'rgba(255,255,255,0.08)', border: '2px solid rgba(255,255,255,0.1)', borderRadius: 14, padding: '10px 14px', cursor: 'pointer', marginBottom: 8, color: '#fff', fontSize: 13 }}>
              ...
              <IC.ChevRight s={15} c="rgba(255,255,255,0.35)" />
            </button>
          ))}
        </div>
      </div>
    ...
    <div style={{ minHeight: '100vh', background: `linear-gradient(150deg, ${B.ink} 0%, #2D1050 60%, #0A1840 100%)`, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, position: 'relative' }}>
      ...
      <div style={{ maxWidth: 420, width: '100%', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.07)', borderRadius: 24, padding: 28, border: '2px solid rgba(255,255,255,0.1)' }}>
          {!isLogin && <div style={{ marginBottom: 14 }}><label style={{ color: 'rgba(255,255,255,0.6)', fontSize: 11, fontWeight: 800, display: 'block', marginBottom: 6, letterSpacing: 0.8 }}>FULL NAME</label><input value={form.name} onChange={e => upd('name', e.target.value)} placeholder="e.g. Tonye Briggs" style={inputSt} /></div>}
          <div style={{ marginBottom: 14 }}><label style={{ color: 'rgba(255,255,255,0.6)', fontSize: 11, fontWeight: 800, display: 'block', marginBottom: 6, letterSpacing: 0.8 }}>EMAIL</label><input value={form.email} onChange={e => upd('email', e.target.value)} placeholder="you@example.com" type="email" style={inputSt} /></div>
          <div style={{ marginBottom: 20 }}><label style={{ color: 'rgba(255,255,255,0.6)', fontSize: 11, fontWeight: 800, display: 'block', marginBottom: 6, letterSpacing: 0.8 }}>PASSWORD</label><input value={form.password} onChange={e => upd('password', e.target.value)} type="password" placeholder={isLogin ? 'Your password' : 'Min. 6 characters'} onKeyDown={e => { if (e.key === 'Enter') isLogin ? handleLogin() : handleSignup() }} style={inputSt} /></div>
          ...
          <Btn onClick={isLogin ? handleLogin : handleSignup} bg={`linear-gradient(135deg, ${B.purple}, ${B.purpleMid})`} style={{ width: '100%', fontSize: 15, padding: '14px 20px' }} disabled={busy}>{busy ? 'Please wait…' : isLogin ? 'Sign In ✨' : 'Create Account 🎉'}</Btn>
          <p style={{ textAlign: 'center', color: 'rgba(255,255,255,0.45)', fontSize: 13, margin: '14px 0 0' }}>{isLogin ? "No account? " : "Already have one? "}<button onClick={() => { setMode(isLogin ? 'signup' : 'login'); setErr('') }} style={{ background: 'none', border: 'none', color: B.purpleLight, cursor: 'pointer', fontWeight: 800, fontSize: 13 }}>{isLogin ? 'Sign up' : 'Sign in'}</button></p>
        </div>
        <button onClick={() => setMode('landing')} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.35)', marginTop: 18, cursor: 'pointer', fontSize: 13, display: 'block', width: '100%', textAlign: 'center' }}>← Back</button>
      </div>
    </div>
  )
}
        ...
        {DIALECTS.map(d => {
          ...
          return (
            <div key={d} onMouseEnter={() => setHov(d)} onMouseLeave={() => setHov(null)} onClick={() => onSelect(d)}
              ...
              <IC.ChevRight s={20} c="rgba(255,255,255,0.4)" />
            </div>
          )
        })}
      </div>
  ...
  return (
    <div style={{ paddingBottom: 40 }}>
      <div style={{ background: `linear-gradient(140deg, ${B.ink} 0%, ${B.inkMid} 50%, #3B0E8C 100%)`, padding: '28px 24px 36px', borderRadius: '0 0 36px 36px', marginBottom: 24, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20, position: 'relative', zIndex: 1 }}>
          <div>
            ...
            <h2 style={{ color: '#fff', fontSize: 26, fontWeight: 900, margin: 0 }}>{completedCount === 0 ? "Let's start! 🌊" : 'Keep going! 🔥'}</h2>
          </div>
          <button onClick={onSwitch} style={{ background: 'rgba(255,255,255,0.12)', border: '2px solid rgba(255,255,255,0.2)', borderRadius: 14, padding: '9px 15px', color: 'rgba(255,255,255,0.9)', cursor: 'pointer', fontSize: 12.5, fontWeight: 800, flexShrink: 0, display: 'flex', alignItems: 'center', gap: 6 }}>
            <IC.Switch s={14} c="rgba(255,255,255,0.9)" />Switch
          </button>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 18, position: 'relative', zIndex: 1 }}>
          {[{ label: 'Level', value: level, color: B.yellow, bg: 'rgba(255,209,102,0.15)' }, { label: 'Streak', value: `${dialectStats.streak}d`, color: B.orange, bg: 'rgba(255,159,28,0.15)' }, { label: 'XP', value: dialectStats.xp, color: B.purpleLight, bg: 'rgba(196,181,253,0.15)' }].map((s, i) => (
      ...
      <div style={{ padding: '0 20px' }}>
        ...
        {nextLesson && (
          <div onClick={() => setScreen('learn')} className="quick-btn"
            style={{ background: `linear-gradient(135deg, ${B.purple} 0%, ${B.purpleMid} 50%, #4C1D95 100%)`, borderRadius: 22, padding: '20px 22px', marginBottom: 20, cursor: 'pointer', position: 'relative', overflow: 'hidden', boxShadow: `0 10px 36px ${B.purple}45`, transition: 'transform 0.22s cubic-bezier(0.34,1.56,0.64,1)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, position: 'relative', zIndex: 1 }}>
              ...
              <IC.ChevRight s={22} c="rgba(255,255,255,0.7)" />
            </div>
          </div>
        )}
        ...
        <Card style={{ marginBottom: 20, padding: '22px 24px', background: `linear-gradient(135deg, ${B.white} 0%, ${B.purplePale} 100%)`, border: `2px solid ${B.purpleLight}50` }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
            ...
            <button onClick={() => speak(wod, apiKey)} style={{ background: B.purple, border: 'none', borderRadius: 14, width: 42, height: 42, cursor: 'pointer', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: `0 4px 14px ${B.purple}40` }}>
              <IC.Volume s={18} c="#fff" />
            </button>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          {[
            { label: 'Learn',      sub: 'All lessons',                       bg: B.purple,  s: 'learn',      icon: '📚', sc: B.purple },
            { label: 'Quiz',       sub: 'Test yourself',                     bg: B.blue,    s: 'quiz',       icon: '🧠', sc: B.blue },
            { label: 'Dictionary', sub: `${getAllWords(dialect).length} words`, bg: B.coral,   s: 'dictionary', icon: '📖', sc: B.coral },
            { label: 'Speak',      sub: 'Practice aloud',                    bg: B.mint,    s: 'voice',      icon: '🎙️', sc: B.mint },
          ].map(item => (
            <button key={item.s} onClick={() => setScreen(item.s)} className="quick-btn"
              style={{ background: `linear-gradient(140deg, ${item.bg}, ${item.bg}cc)`, border: 'none', borderRadius: 20, padding: '20px 18px', cursor: 'pointer', textAlign: 'left', boxShadow: `0 6px 22px ${item.sc}35`, transition: 'transform 0.2s' }}>
              <div style={{ fontSize: 26, marginBottom: 8 }}>{item.icon}</div>
              <p style={{ color: '#fff', fontWeight: 900, fontSize: 16, margin: '0 0 3px' }}>{item.label}</p>
              <p style={{ color: 'rgba(255,255,255,0.65)', fontSize: 12, margin: 0, fontWeight: 600 }}>{item.sub}</p>
            </button>
          ))}
        </div>
      </div>
              ...
              <div style={{ flex: 1, marginLeft: 12, paddingBottom: 10 }}>
                <Card hover={status !== 'locked'} onClick={status === 'locked' ? undefined : () => { setPlayerContext({ lesson: l, dialect }); setScreen('player') }} style={{ opacity: status === 'locked' ? 0.4 : 1, cursor: status === 'locked' ? 'not-allowed' : 'pointer', border: status === 'available' ? `2px solid ${B.purple}` : undefined }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    {status !== 'locked' && <IC.ChevRight s={18} c={B.inkFaint} />}
                  </div>
                </Card>
              </div>
            </div>
  ...
  if (done) {
    ...
    return (
      <div style={{ minHeight: '100vh', background: `linear-gradient(160deg, ${B.ink}, #1A0840)`, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 32 }}>
        ...
        <button onClick={() => setScreen('learn')} style={{ color: 'rgba(255,255,255,0.35)', background: 'none', border: 'none', cursor: 'pointer', fontSize: 14, fontWeight: 600 }}>Back to lessons</button>
      </div>
    )
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: `linear-gradient(160deg, ${B.ink} 0%, #1A0840 100%)`, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 12 }}>
        <button onClick={() => setScreen('learn')} style={{ background: 'rgba(255,255,255,0.1)', border: '2px solid rgba(255,255,255,0.12)', borderRadius: 12, width: 40, height: 40, color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <IC.ArrowLeft s={18} c="rgba(255,255,255,0.8)" />
        </button>
        <div style={{ flex: 1 }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '16px 28px' }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 16, width: '100%', maxWidth: 430 }}>
          <Btn onClick={() => speak(w, apiKey)} bg="rgba(255,255,255,0.12)" color="#fff" style={{ flex: 1, border: '2px solid rgba(255,255,255,0.18)' }}><IC.Volume s={16} c="#fff" />Audio</Btn>
          <Btn onClick={() => { setFlipped(false); speak(w, apiKey) }} bg="rgba(255,255,255,0.12)" color="#fff" style={{ flex: 1, border: '2px solid rgba(255,255,255,0.18)' }}><IC.Repeat s={16} c="#fff" />Again</Btn>
        </div>
        {flipped && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 14, width: '100%', maxWidth: 430 }}>
            <button onClick={() => mark(false)} style={{ background: 'rgba(239,68,68,0.18)', border: '2px solid rgba(239,68,68,0.4)', color: '#fff', borderRadius: 18, padding: 16, fontSize: 16, fontWeight: 900, cursor: 'pointer' }}>Again 😅</button>
            <button onClick={() => mark(true)} style={{ background: 'rgba(6,214,160,0.2)', border: '2px solid rgba(6,214,160,0.45)', color: '#fff', borderRadius: 18, padding: 16, fontSize: 16, fontWeight: 900, cursor: 'pointer' }}>Got it! ✨</button>
          </div>
        )}
      </div>
    </div>
            ...
            <Card key={l.stage} hover={prevDone} onClick={prevDone ? () => startLesson(l) : undefined} style={{ marginBottom: 12, opacity: prevDone ? 1 : 0.4, cursor: prevDone ? 'pointer' : 'not-allowed' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 48, height: 48, borderRadius: 16, flexShrink: 0, fontSize: 22, display: 'flex', alignItems: 'center', justifyContent: 'center', background: completed ? `${B.mint}18` : prevDone ? B.purplePale : B.surface, border: `2px solid ${completed ? B.mint + '40' : prevDone ? B.purpleLight + '60' : B.border}` }}>
                  {completed ? '✅' : prevDone ? l.icon : <IC.Lock s={16} c={B.inkFaint} />}
                </div>
                <div style={{ flex: 1 }}>
                  <p style={{ margin: 0, fontWeight: 900, fontSize: 15, color: B.ink }}>Stage {l.stage}: {l.title}</p>
    ...
    <div style={{ padding: '0 20px 80px' }}>
      ...
      <div style={{ maxWidth: 560, margin: '0 auto', paddingTop: isMobile ? 20 : 8 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: passed ? '1fr' : '1fr 1fr', gap: 10, marginTop: 16 }}>
          {!passed && <Btn onClick={retry}><IC.Repeat s={15} c="#fff" />Retry</Btn>}
      ...
      <div style={{ maxWidth: 560, margin: '0 auto', paddingTop: isMobile ? 16 : 0 }}>
        {!isMobile && (
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            ...
            <button onClick={() => setSelLesson(null)} style={{ background: 'none', border: 'none', color: B.purple, cursor: 'pointer', fontWeight: 800, fontSize: 13 }}>All quizzes</button>
          </div>
        )}
        ...
        {q.opts.map((opt, i) => {
          ...
          return (
            <div key={i} onClick={() => choose(opt)} style={{ background: bg, border, borderRadius: 16, padding: '15px 18px', marginBottom: 10, cursor: sel === null ? 'pointer' : 'default', display: 'flex', alignItems: 'center', gap: 13, transition: 'all 0.2s', color: tc }}>
              <div style={{ width: 32, height: 32, borderRadius: 10, border: `2px solid ${sel !== null && opt === q.correct ? B.green : sel !== null && opt === sel ? B.red : B.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, fontSize: 13, flexShrink: 0, color: tc }}>{['A', 'B', 'C', 'D'][i]}</div>
  ...
  if (sel) return (
    <div>
      ...
      <div style={{ padding: '0 20px 80px', maxWidth: 560, margin: '0 auto' }}>
        {!isMobile && <button onClick={() => setSel(null)} style={{ background: 'none', border: 'none', color: B.purple, cursor: 'pointer', fontSize: 13.5, fontWeight: 800, margin: '8px 0 16px', display: 'flex', alignItems: 'center', gap: 6 }}><IC.ArrowLeft s={14} c={B.purple} />Dictionary</button>}
        ...
        <Btn onClick={() => speak(sel, apiKey)} style={{ width: '100%', marginBottom: 10 }}><IC.Volume s={16} c="#fff" />Hear Pronunciation</Btn>
        <Btn onClick={() => setScreen('voice')} outline style={{ width: '100%' }}><IC.Voice s={16} c={B.purple} />Practice This Word</Btn>
      </div>
    </div>
  )
    ...
    <div style={{ padding: '0 20px 80px' }}>
      ...
      <div style={{ maxWidth: 700, margin: '0 auto', paddingTop: isMobile ? 16 : 0 }}>
        ...
        <div style={{ position: 'relative', marginBottom: 14 }}>
          <div style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)' }}><IC.Search s={17} c={B.inkFaint} /></div>
          <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search Ijaw or English…" style={{ width: '100%', padding: '13px 16px 13px 42px', borderRadius: 16, border: `2px solid ${B.border}`, fontSize: 15, background: B.white, boxSizing: 'border-box', outline: 'none', color: B.ink }} />
        </div>
            ...
            <Card key={w.id} hover onClick={() => setSel(w)} style={{ cursor: 'pointer', padding: '15px 17px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                <button onClick={e => { e.stopPropagation(); speak(w, apiKey) }} style={{ background: B.purplePale, border: `2px solid ${B.border}`, borderRadius: 12, width: 36, height: 36, cursor: 'pointer', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: 2 }}>
                  <IC.Volume s={15} c={B.purple} />
                </button>
              </div>
            </Card>
          ))}
        </div>
        {filtered.length === 0 && <div style={{ textAlign: 'center', padding: 64, color: B.inkFaint }}><Bird size={72} /><p style={{ fontWeight: 800, marginTop: 14, fontSize: 16 }}>No words found 🤔</p></div>}
      </div>
    </div>
  )
}
...
function Voice({ dialect, isMobile, updateDialectStats, onSwitch, addXP, apiKey, grantAchievement }) {
  ...
  const mediaRef = useRef(null); const chunksRef = useRef([]); const blobRef = useRef(null)
    ...
    <div style={{ paddingBottom: 80 }}>
      ...
      <div style={{ padding: '0 20px', maxWidth: 640, margin: '0 auto', paddingTop: isMobile ? 16 : 0 }}>
        ...
        <Card style={{ background: `linear-gradient(135deg, ${B.purple} 0%, ${B.purpleMid} 60%, #4C1D95 100%)`, textAlign: 'center', padding: '42px 32px', marginBottom: 20, border: 'none', boxShadow: `0 12px 40px ${B.purple}45` }}>
          ...
          <button onClick={() => speak(curr, apiKey)} style={{ background: 'rgba(255,255,255,0.18)', border: '2px solid rgba(255,255,255,0.3)', borderRadius: 50, padding: '12px 28px', color: '#fff', cursor: 'pointer', fontSize: 14.5, fontWeight: 800, display: 'flex', alignItems: 'center', gap: 8, margin: '0 auto' }}>
            <IC.Volume s={17} c="#fff" />Hear Sample
          </button>
        </Card>
        {micDenied && <Card style={{ background: `${B.yellow}15`, border: `2px solid ${B.yellow}50`, marginBottom: 14, padding: '14px 16px' }}><p style={{ margin: 0, fontSize: 13.5, color: B.ink, fontWeight: 800 }}>🔒 Mic access blocked</p><p style={{ margin: '5px 0 0', fontSize: 12.5, color: B.inkSoft }}>Allow mic in your browser&apos;s address bar settings.</p></Card>}
        {score !== null && (
        ...
        <Card style={{ marginBottom: 18 }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {pw.map((word, i) => (
              <button key={word.id} onClick={() => selectWord(i)} style={{ padding: '8px 15px', borderRadius: 20, border: `2px solid ${i === wi ? B.purple : B.border}`, cursor: 'pointer', background: i === wi ? B.purple : B.offWhite, color: i === wi ? '#fff' : B.inkSoft, fontWeight: 800, fontSize: 13, transition: 'all 0.18s cubic-bezier(0.34,1.56,0.64,1)' }}>{word.ijaw}</button>
            ))}
          </div>
        </Card>
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', paddingTop: isMobile ? 16 : 0 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(175px,1fr))', gap: 12, marginBottom: 26 }}>
          {DIALECTS.map(d => {
            ...
            return (
              <Card key={d} style={{ border: d === dialect ? `2px solid ${B.purple}` : undefined, padding: '16px 17px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(158px,1fr))', gap: 12, marginBottom: 26 }}>
          {ALL_ACHIEVEMENTS.map(a => {
            const earned = (ds.achievements || []).includes(a.id)
              || (a.id === 'first_lesson' && done > 0)
              || (a.id === 'quiz_pass' && Object.keys(ds.completedLessons).length > 0)
              || (a.id === 'all_stages' && done === dlessons.length && dlessons.length > 0)
            return (
        ...
        <button onClick={onLogout} style={{ width: '100%', padding: 14, borderRadius: 16, border: `2px solid ${B.coralLight}`, background: 'transparent', color: B.coral, cursor: 'pointer', fontWeight: 800, fontSize: 15, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9 }}>
          <IC.Logout s={17} c={B.coral} />Sign Out
        </button>
      </div>
    </div>
  )
...
export default function App() {
  ...
  const updateDialectStats = useCallback((updater) => {
    setAllStats(all => ({ ...all, [dialect]: updater(all[dialect] || freshStats()) }))
  }, [dialect])
  const addXP = useCallback((amt) => {
    updateDialectStats(d => ({ ...d, xp: d.xp + amt }))
    setXpToast(amt)
    setTimeout(() => setXpToast(null), 2200)
    doStreak()
  }, [updateDialectStats, doStreak])
  ...
  if (!authUser) return <><GlobalStyles /><AuthScreen onLogin={handleLogin} /></>
  if (!hasChosen) return <><GlobalStyles /><DialectPicker onSelect={handleDialectPick} /></>
  if (screen === 'player' && playerCtx) return (
    <><GlobalStyles /><LessonPlayer context={playerCtx} apiKey={apiKey} grantAchievement={grantAchievement}
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: B.offWhite }}>
      <GlobalStyles />
        ...
        {!isMobile && (
          <div style={{ padding: '15px 28px 13px', borderBottom: `2px solid ${B.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: B.white, position: 'sticky', top: 0, zIndex: 100 }}>
            <div>
              ...
              <p style={{ margin: '2px 0 0', fontSize: 11.5, color: B.inkFaint, fontWeight: 700 }}>
                {DIALECT_INFO[dialect].flag} {dialect} · Level {calcLevel(dialectStats.xp)} · {dialectStats.xp} XP
              </p>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ position: 'relative' }}>
                <button onClick={() => setShowStreakPop(p => !p)} style={{ background: 'none', border: `2px solid ${B.border}`, borderRadius: 12, cursor: 'pointer', padding: '6px 12px', display: 'flex', alignItems: 'center', gap: 4 }}>
              ...
              <button onClick={() => setShowSpeech(true)} style={{ background: apiKey ? `${B.mint}15` : B.surface, border: `2px solid ${apiKey ? B.mint + '50' : B.border}`, borderRadius: 20, padding: '7px 14px', cursor: 'pointer', fontWeight: 800, fontSize: 12, color: apiKey ? B.mint : B.inkFaint, display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <button onClick={() => setShowModal(true)} style={{ background: B.purplePale, border: `2px solid ${B.purpleLight}60`, borderRadius: 20, padding: '8px 15px', cursor: 'pointer', fontWeight: 800, fontSize: 13, color: B.purple, display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 16 }}>{DIALECT_INFO[dialect].flag}</span>{dialect}<IC.ChevDown s={13} c={B.purple} />
              </button>
            </div>
          </div>
        )}
      ...
      {showSpeech    && <SpeechModal      apiKey={apiKey}   onSave={saveApiKey}  onClose={() => setShowSpeech(false)} />}
