...
/* ─── CONSTANTS ─────────────────────────────────────────────── */
const DIALECTS = ["Izon", "Kalabari", "Nembe"];
...
function freshAllStats() {
  return { Izon:freshStats(), Kalabari:freshStats(), Nembe:freshStats() };
}
...
function getLessons(dialect) {
  return RAW_LESSONS
    .map(l => ({ ...l, words: l.words[dialect] || [] }))
    .filter(l => l.words.length > 0);
}
function getAllWords(dialect) {
  return getLessons(dialect).flatMap(l => l.words);
}
function buildQuiz(lesson, dialect) {
  ...
  return lesson.words.map(w => {
    const wrong = pool.filter(x => x.id !== w.id)
      .sort(() => Math.random() - 0.5).slice(0, 3).map(x => x.english);
...
async function speakElevenLabs(phonetic, apiKey) {
  ...
  if (!blob) {
    const target = `https://api.elevenlabs.io/v1/text-to-speech/${VOICE_ID}`;
...
async function speak(word, apiKey) {
  ...
  if (apiKey) {
    try { await speakElevenLabs(phonetic, apiKey); return; }
    catch (e) { console.warn("ElevenLabs:", e.message); }
  }
...
function Btn({ children, onClick, color, style, disabled, outline }) {
  ...
  return (
    <button
      ...
      style={{
        background: outline ? "transparent" : (disabled ? "#D1C9E8" : (h ? B.purpleDark : c)),
        color: outline ? c : "#fff",
        border: outline ? `2px solid ${c}` : "none",
        borderRadius:14, padding:"13px 22px", fontSize:15, fontWeight:700,
        cursor: disabled ? "not-allowed" : "pointer",
        display:"flex", alignItems:"center", justifyContent:"center", gap:8,
        transition:"all 0.18s", letterSpacing:0.3,
        boxShadow: (!outline && !disabled) ? `0 3px 10px ${c}33` : "none",
        ...s,
      }}>{children}</button>
  );
}
...
function Badge({ children, color }) {
  const c = color || B.purple;
  return (
    <span style={{ background:`${c}18`, color:c, border:`1px solid ${c}33`, borderRadius:20, padding:"3px 10px", fontSize:11, fontWeight:700 }}>
      {children}
    </span>
  );
}
...
function PBar({ value, max, color, h, style }) {
  ...
  return (
    <div style={{ background:B.border, borderRadius:99, height, overflow:"hidden", ...s }}>
      <div style={{ width:`${pct}%`, height:"100%", background:c, borderRadius:99, transition:"width 0.5s" }} />
    </div>
  );
}
...
function Bird({ size, bob, style }) {
  ...
  return (
    <svg width={sz} height={sz} viewBox="0 0 200 200"
      style={{ ...s, ...(bob ? { animation:"birdBob 2.2s ease-in-out infinite" } : {}) }}>
...
function SpeechSettingsModal({ apiKey, onSave, onClose }) {
  ...
  async function testKey() {
    ...
    try {
      await speakElevenLabs("Owari", val);
      setResult("success");
    } catch (e) {
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(30,10,60,0.75)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }} onClick={onClose}>
      <div style={{ background:B.white, borderRadius:24, padding:28, maxWidth:420, width:"100%", boxShadow:"0 24px 80px rgba(107,33,232,0.3)" }} onClick={e => e.stopPropagation()}>
        <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:16 }}>
          ...
          <div>
            <h3 style={{ margin:0, fontSize:18, color:B.ink }}>Localized Voice</h3>
            <p style={{ margin:0, fontSize:12, color:B.inkLight }}>Powered by ElevenLabs</p>
          </div>
        </div>
        <div style={{ background:B.purplePale, borderRadius:12, padding:"12px 14px", marginBottom:16, fontSize:13, color:B.inkMid, lineHeight:1.6 }}>
          Browser voices don't support Nigerian or Ijaw accents. ElevenLabs' multilingual model handles West African phonemes using roman phonetic input.
          <br/><br/>Get a free key at <strong>elevenlabs.io</strong> (10,000 chars/month free).
        </div>
        ...
        {result && (
          <p style={{ fontSize:13, margin:"0 0 10px", fontWeight:700, color:result==="success" ? B.green : B.red }}>
            {result === "success" ? "✓ Voice working!" : `✗ ${result.replace("fail:","")}`}
          </p>
        )}
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:10 }}>
          <button onClick={testKey} disabled={!val || testing}
            style={{ padding:11, borderRadius:12, border:`2px solid ${B.purple}`, background:"transparent", color:B.purple, cursor:(!val||testing)?"not-allowed":"pointer", fontWeight:700, fontSize:14 }}>
            {testing ? "Testing…" : "▶ Test"}
          </button>
          <button onClick={() => { onSave(val); onClose(); }}
            style={{ padding:11, borderRadius:12, border:"none", background:B.purple, color:"#fff", cursor:"pointer", fontWeight:700, fontSize:14 }}>
            Save & Close
          </button>
        </div>
        {val && (
          <button onClick={() => { onSave(""); onClose(); }}
            style={{ width:"100%", padding:10, borderRadius:12, border:`1px solid ${B.border}`, background:"transparent", color:B.inkLight, cursor:"pointer", fontSize:13 }}>
            Remove key (use browser voice)
          </button>
        )}
        <p style={{ fontSize:11, color:B.inkLight, margin:"12px 0 0", textAlign:"center" }}>Key stored in localStorage only — never sent anywhere except ElevenLabs.</p>
      </div>
    </div>
  );
}
...
function Sidebar({ screen, setScreen, dialect, onSwitch, stats, user, onLogout }) {
  ...
  return (
    <div style={{ width:col?68:228, minHeight:"100vh", background:`linear-gradient(180deg,${B.purpleDark} 0%,#0D0420 100%)`, display:"flex", flexDirection:"column", transition:"width 0.25s", flexShrink:0, position:"sticky", top:0, height:"100vh" }}>
      ...
      {!col && (
        <button onClick={onLogout} style={{ background:"rgba(220,38,38,0.15)", border:"none", color:"rgba(255,100,100,0.8)", padding:"12px 14px", cursor:"pointer", fontSize:13, fontWeight:700, textAlign:"left", display:"flex", alignItems:"center", gap:8, borderTop:"1px solid rgba(255,255,255,0.07)" }}>
          🚪 Sign Out
        </button>
      )}
      <button onClick={() => setCol(!col)} style={{ background:"rgba(255,255,255,0.05)", border:"none", color:"rgba(255,255,255,0.4)", padding:14, cursor:"pointer", fontSize:13, borderTop:"1px solid rgba(255,255,255,0.07)" }}>
        {col ? "→" : "← Collapse"}
      </button>
    </div>
  );
    ...
    <div style={{ position:"fixed", bottom:0, left:0, right:0, zIndex:200, background:B.white, borderTop:`2px solid ${B.border}`, display:"flex", justifyContent:"space-around", padding:"6px 0 max(8px,env(safe-area-inset-bottom))" }}>
      {NAV.map(n => (
        <button key={n.id} onClick={() => setScreen(n.id)}
          style={{ background:"none", border:"none", cursor:"pointer", padding:"4px", display:"flex", flexDirection:"column", alignItems:"center", gap:2, flex:1, color:screen===n.id?B.purple:B.inkLight }}>
...
function TopBar({ title, onBack, dialect, onSwitch, right }) {
  return (
    <div style={{ display:"flex", alignItems:"center", padding:"14px 16px 12px", background:B.white, borderBottom:`1.5px solid ${B.border}`, position:"sticky", top:0, zIndex:100 }}>
      {onBack && (
        <button onClick={onBack} style={{ background:B.purplePale, border:"none", borderRadius:10, width:36, height:36, fontSize:16, cursor:"pointer", color:B.purple, marginRight:10, display:"flex", alignItems:"center", justifyContent:"center" }}>←</button>
      )}
      <span style={{ fontSize:17, fontWeight:800, color:B.ink, flex:1 }}>{title}</span>
      {dialect && (
        <button onClick={onSwitch} style={{ background:B.purplePale, border:"none", borderRadius:20, padding:"6px 12px", fontSize:12, fontWeight:700, cursor:"pointer", color:B.purple, marginRight:8 }}>
      ...
      <div style={{ background:B.white, borderRadius:28, padding:28, maxWidth:420, width:"100%", boxShadow:"0 24px 80px rgba(107,33,232,0.3)" }} onClick={e => e.stopPropagation()}>
        <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:8 }}>
          ...
          <div>
            ...
            <p style={{ color:B.inkLight, fontSize:13, margin:0 }}>Progress tracked independently per dialect</p>
          </div>
        </div>
        <p style={{ color:B.inkLight, fontSize:13, margin:"0 0 16px", background:B.purplePale, borderRadius:10, padding:"8px 12px" }}>
          💡 Switching won't reset progress — each dialect saves separately.
        </p>
        {DIALECTS.map(d => {
          const ds = allStats[d] || freshStats();
          const dl = getLessons(d);
          const done = dl.filter(l => ds.completedLessons[`${d}_${l.stage}`]).length;
          return (
            <div key={d} onClick={() => onSelect(d)}
              style={{ display:"flex", alignItems:"center", gap:14, padding:"14px 16px", borderRadius:16, marginBottom:10, cursor:"pointer", background:d===dialect?B.purplePale:B.off, border:`2px solid ${d===dialect?B.purple:B.border}`, transition:"all 0.15s" }}>
...
function AuthScreen({ onLogin }) {
  ...
  function handleSignup() {
    ...
    const db = getUsersDB();
    if (Object.values(db).find(u => u.email.toLowerCase() === form.email.toLowerCase())) {
      setErr("Account already exists. Sign in instead."); return;
    }
    setBusy(true);
    setTimeout(() => {
      const avatars = ["👩🏾","👨🏿","👩🏿","👦🏾","👧🏿","🧑🏾"];
  ...
  if (mode === "landing") return (
    <div style={{ minHeight:"100vh", background:`linear-gradient(150deg,${B.purpleDark} 0%,#3B0E8C 50%,#0D2040 100%)`, display:"flex", alignItems:"center", justifyContent:"center", padding:24 }}>
      ...
      <div style={{ maxWidth:440, width:"100%", textAlign:"center" }}>
        ...
        <p style={{ color:"rgba(255,255,255,0.6)", fontSize:15, marginBottom:40, lineHeight:1.6 }}>Reconnect with your heritage.<br />Learn Ijaw languages through audio, quizzes & voice practice.</p>
        ...
        <div style={{ background:"rgba(255,255,255,0.07)", borderRadius:16, padding:"16px 20px", border:"1px solid rgba(255,255,255,0.1)" }}>
          ...
          {MOCK_USERS.map(u => (
            <button key={u.id} onClick={() => { setForm({ email:u.email, password:u.password, name:u.name }); setMode("login"); }}
              style={{ display:"flex", alignItems:"center", gap:10, width:"100%", background:"rgba(255,255,255,0.08)", border:"1px solid rgba(255,255,255,0.12)", borderRadius:10, padding:"9px 12px", cursor:"pointer", marginBottom:6, color:"#fff", fontSize:13 }}>
              <span style={{ fontSize:22 }}>{u.avatar}</span>
              <div style={{ flex:1, textAlign:"left" }}>
                <p style={{ margin:0, fontWeight:700 }}>{u.name}</p>
                <p style={{ margin:0, fontSize:11, opacity:0.6 }}>{u.email}</p>
              </div>
        ...
        <div style={{ background:"rgba(255,255,255,0.07)", borderRadius:24, padding:28, border:"1px solid rgba(255,255,255,0.12)" }}>
          {!isLogin && (
            <div style={{ marginBottom:16 }}>
              <label style={{ color:"rgba(255,255,255,0.7)", fontSize:13, fontWeight:700, display:"block", marginBottom:6 }}>Full Name</label>
              <input value={form.name} onChange={e => updateForm("name", e.target.value)} placeholder="e.g. Tonye Briggs" style={inputStyle} />
            </div>
          )}
          <div style={{ marginBottom:16 }}>
            <label style={{ color:"rgba(255,255,255,0.7)", fontSize:13, fontWeight:700, display:"block", marginBottom:6 }}>Email</label>
            <input value={form.email} onChange={e => updateForm("email", e.target.value)} placeholder="you@example.com" type="email" style={inputStyle} />
          </div>
          <div style={{ marginBottom:20 }}>
            <label style={{ color:"rgba(255,255,255,0.7)", fontSize:13, fontWeight:700, display:"block", marginBottom:6 }}>Password</label>
          ...
          <Btn onClick={isLogin ? handleLogin : handleSignup} color={B.purple} style={{ width:"100%", fontSize:16, padding:15 }} disabled={busy}>
            {busy ? "⏳ Please wait…" : isLogin ? "Sign In →" : "Create Account →"}
          </Btn>
          <p style={{ textAlign:"center", color:"rgba(255,255,255,0.5)", fontSize:13, margin:"16px 0 0" }}>
            {isLogin ? "Don't have an account? " : "Already have an account? "}
            <button onClick={() => { setMode(isLogin ? "signup" : "login"); setErr(""); }}
              style={{ background:"none", border:"none", color:B.purpleLight, cursor:"pointer", fontWeight:700, fontSize:13 }}>
    ...
    <div style={{ paddingBottom:40 }}>
      ...
      <div style={{ background:`linear-gradient(135deg,${B.purpleDark} 0%,${B.purpleMid} 70%,#4C1D95 100%)`, padding:"28px 24px 36px", borderRadius:"0 0 36px 36px", marginBottom:24, position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:18 }}>
          ...
          {/* ← Switch button wired directly to onSwitch */}
          <button
            onClick={onSwitch}
            style={{ background:"rgba(255,255,255,0.15)", border:"1px solid rgba(255,255,255,0.3)", borderRadius:12, padding:"9px 16px", color:"#fff", cursor:"pointer", fontSize:13, fontWeight:700, flexShrink:0, zIndex:1 }}>
            Switch ↕
          </button>
        </div>
...
function Learn({ dialect, setScreen, dialectStats, isMobile, onSwitch, setPlayerContext }) {
  ...
  function getStatus(l) {
    ...
    const prevOk = l.stage === 1 || dialectStats.completedLessons[`${dialect}_${l.stage-1}`];
          ...
          return (
            <div key={l.stage} style={{ display:"flex", gap:0, marginBottom:8, alignItems:"stretch" }}>
              ...
              <div style={{ flex:1, marginLeft:12, paddingBottom:12 }}>
                <Card hover={status!=="locked"} onClick={status==="locked" ? undefined : () => { setPlayerContext({ lesson:l, dialect }); setScreen("player"); }}
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                    <div style={{ width:48, height:48, borderRadius:16, flexShrink:0, color:"#fff", fontSize:22, background:status==="done"?B.green:status==="available"?`linear-gradient(135deg,${B.purple},${B.purpleMid})`:"#D1C9E8", display:"flex", alignItems:"center", justifyContent:"center" }}>
                      {status==="done" ? "✓" : l.icon}
                    </div>
                    <div style={{ flex:1 }}>
...
/* ─── LESSON PLAYER ─────────────────────────────────────────── */
function LessonPlayer({ context, setScreen, addXP, apiKey }) {
...
/* ─── QUIZ ──────────────────────────────────────────────────── */
function Quiz({ lessonContext, setScreen, dialect, dialectStats, updateDialectStats, addXP, isMobile, onSwitch, apiKey }) {
  const dlessons = getLessons(dialect);
  ...
  function startLesson(l) {
    setSelLesson(l); setQs(buildQuiz(l, dialect).sort(() => Math.random()-0.5));
    ...
    <div style={{ paddingBottom:80 }}>
      ...
      <div style={{ padding:`${isMobile?16:0}px 20px 60px`, maxWidth:600, margin:"0 auto" }}>
        ...
        {dlessons.map(l => {
          ...
          const completed = !!dialectStats.completedLessons[key];
          const prevDone  = l.stage===1 || !!dialectStats.completedLessons[`${dialect}_${l.stage-1}`];
          return (
            <Card key={l.stage} hover={prevDone} onClick={prevDone ? () => startLesson(l) : undefined}
              style={{ marginBottom:12, opacity:prevDone?1:0.45, cursor:prevDone?"pointer":"not-allowed" }}>
              <div style={{ display:"flex", alignItems:"center", gap:14 }}>
                <div style={{ width:48, height:48, borderRadius:16, flexShrink:0, fontSize:22, color:"#fff", background:completed?B.green:prevDone?`linear-gradient(135deg,${B.purple},${B.purpleMid})`:"#D1C9E8", display:"flex", alignItems:"center", justifyContent:"center" }}>
    ...
    <div style={{ padding:"0 20px 80px" }}>
      ...
      <div style={{ maxWidth:560, margin:"0 auto", paddingTop:isMobile?16:0 }}>
        ...
        {q.opts.map((opt, i) => {
          ...
          return (
            <div key={i} onClick={() => choose(opt)} style={{ background:bg, border, borderRadius:16, padding:"16px 18px", marginBottom:10, cursor:sel===null?"pointer":"default", display:"flex", alignItems:"center", gap:14, transition:"all 0.2s", color:tc }}>
          ...
          {filtered.map(w => (
            <Card key={w.id} hover onClick={() => setSel(w)} style={{ cursor:"pointer", padding:"14px 18px" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                ...
                <button onClick={e => { e.stopPropagation(); speak(w, apiKey); }}
                  style={{ background:B.purplePale, border:`1px solid ${B.purpleLight}`, borderRadius:50, width:36, height:36, fontSize:14, cursor:"pointer", flexShrink:0, color:B.purple, display:"flex", alignItems:"center", justifyContent:"center" }}>▶</button>
              </div>
            </Card>
          ))}
        </div>
...
/* ─── VOICE ─────────────────────────────────────────────────── */
function Voice({ dialect, isMobile, updateDialectStats, onSwitch, addXP, apiKey }) {
  ...
  const finish = useCallback(() => {
    ...
    if (n === 1) {
      updateDialectStats(d => ({ ...d, achievements:[...new Set([...(d.achievements||[]), "voice_rec"])] }));
      addXP(15);
    } else { addXP(5); }
  }, [recCount, addXP, updateDialectStats]);
  ...
  async function startRec() {
    ...
    try {
      ...
      mr.onstop = () => {
        ...
        setHasRec(true); stream.getTracks().forEach(t => t.stop());
      ...
      <div style={{ padding:"0 20px", maxWidth:640, margin:"0 auto", paddingTop:isMobile?16:0 }}>
        ...
        {micDenied && (
          <Card style={{ background:"#FEF9C3", border:`2px solid ${B.gold}`, marginBottom:16, padding:"14px 18px" }}>
            ...
            <p style={{ margin:"6px 0 0", fontSize:12, color:B.inkLight }}>Click 🔒 in your browser address bar → Site settings → Microphone → Allow.</p>
          </Card>
        )}
        {score !== null && (
...
function Progress({ dialect, allStats, isMobile, onSwitch, user, onLogout }) {
  ...
  const level   = calcLevel(ds.xp);
  const nxt     = xpToNext(ds.xp);
  const dlessons = getLessons(dialect);
  ...
  function isEarned(a) {
    if (a.id === "all_stages")   return done === dlessons.length && dlessons.length > 0;
    if (a.id === "first_lesson") return done > 0;
    if (a.id === "quiz_pass")    return Object.keys(ds.completedLessons).length > 0;
    return (ds.achievements || []).includes(a.id);
  }
    ...
    <div style={{ padding:"0 20px 80px" }}>
      ...
      <div style={{ maxWidth:640, margin:"0 auto", paddingTop:isMobile?16:0 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))", gap:12, marginBottom:24 }}>
          {DIALECTS.map(d => {
            const dst = allStats[d] || freshStats();
            const dl  = getLessons(d);
...
export default function App() {
  ...
  const [dialect,     setDialect    ] = useState("Izon");
  ...
  const addXP = useCallback((amt) => {
    ...
    setXpToast(amt); setTimeout(() => setXpToast(null), 2200);
  }, [updateDialectStats]);

  function handleLogin(user) {
    setAuthUser(user);
    safeSet("nume_session", user.id);
    setAllStats(loadUserStats(user.id));
    setHasChosen(false);
  }
  function handleLogout() {
    if (authUser) saveUserStats(authUser.id, allStats);
    safeSet("nume_session", null);
    setAuthUser(null); setHasChosen(false); setScreen("home");
    setAllStats(freshAllStats());
  }
  ...
  if (screen === "player" && playerCtx) {
    return (
      <LessonPlayer context={playerCtx} apiKey={apiKey}
        setScreen={s => { if (s === "quizFromLesson") { setLessonCtx(playerCtx.lesson); setScreen("quiz"); } else setScreen(s); }}
        addXP={addXP} />
    );
  }

  const shared = { dialect, setScreen, isMobile, dialectStats, updateDialectStats, allStats, addXP, apiKey, onSwitch:() => setShowModal(true), user:authUser };
          ...
          <div style={{ padding:"18px 28px 14px", borderBottom:`1.5px solid ${B.border}`, display:"flex", justifyContent:"space-between", alignItems:"center", background:B.white, position:"sticky", top:0, zIndex:100 }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              ...
              <button onClick={() => setShowSpeech(true)}
                style={{ background:apiKey?`${B.green}18`:B.purplePale, border:`1px solid ${apiKey?B.green:B.border}`, borderRadius:20, padding:"8px 14px", cursor:"pointer", fontWeight:700, fontSize:12, color:apiKey?B.green:B.inkLight, display:"flex", alignItems:"center", gap:6 }}>
                🎙️ {apiKey ? "Voice ✓" : "Set Voice"}
              </button>
              ...
              <button onClick={() => setShowModal(true)}
                style={{ background:B.purplePale, border:"none", borderRadius:20, padding:"9px 16px", cursor:"pointer", fontWeight:700, fontSize:13, color:B.purple, display:"flex", alignItems:"center", gap:8 }}>
                {DIALECT_INFO[dialect].flag} {dialect} ▾
              </button>
            </div>
          </div>
