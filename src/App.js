...
const RAW_LESSONS = [
  {
    stage:1, title:"Greetings", icon:"👋", xp:20,
    desc:"Everyday greetings and polite phrases",
    words:{
      Kalabari:[
  ...
  {
    stage:2, title:"Numbers", icon:"🔢", xp:20,
    desc:"Count from one to ten",
    words:{
      Kalabari:[
  ...
  {
    stage:3, title:"Family", icon:"👨‍👩‍👧", xp:25,
  ...
  {
    stage:4, title:"Food & Drink", icon:"🍲", xp:25,
    desc:"Traditional food and drinks",
    words:{
      Kalabari:[
  ...
  {
    stage:5, title:"Home & Place", icon:"🏠", xp:30,
    desc:"House, village and surroundings",
    words:{
      Kalabari:[
  ...
  {
    stage:6, title:"Time & Days", icon:"⏰", xp:30,
    desc:"Telling time and days",
    words:{
      Kalabari:[
...
/* ─── SHARED COMPONENTS ─────────────────────────────────────── */
function Bird({ size = 40, bob = false, style = {} }) {
  return (
    <svg
...
function Btn({ children, onClick, color = B.purple, style = {}, disabled = false, outline = false }) {
  const [h, setH] = useState(false);
  return (
    <button
...
function Badge({ children, color = B.purple }) {
  return (
    <span style={{
...
function PBar({ value, max, color = B.purple, h = 8, style = {} }) {
...
function BotNav({ screen, setScreen }) {
  return (
    <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 200, background: B.white, borderTop: `2px solid ${B.border}`, display: "flex", justifyContent: "space-around", padding: "6px 0 max(8px,env(safe-area-inset-bottom))" }}>
      {NAV.map((n) => (
        <button key={n.id} onClick={() => setScreen(n.id)} style={{ background: "none", border: "none", cursor: "pointer", padding: "4px 4px", display: "flex", flexDirection: "column", alignItems: "center", gap: 2, flex: 1, color: screen === n.id ? B.purple : B.inkLight }}>
  ...
  const handleSignup = () => {
    ...
    setTimeout(() => {
      ...
      const newUser = {
        ...
        password: form.password, joinedDate: "Today",
    ...
    return (
      <div style={{ minHeight: "100vh", background: `linear-gradient(150deg,${B.purpleDark} 0%,#3B0E8C 50%,#0D2040 100%)`, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
        ...
        <div style={{ maxWidth: 440, width: "100%", textAlign: "center" }}>
          <div style={{ display: "inline-block", animation: "birdBob 2.5s ease-in-out infinite", marginBottom: 20 }}>
            <Bird size={120} />
          </div>
          ...
          <p style={{ color: "rgba(255,255,255,0.6)", fontSize: 15, marginBottom: 40, lineHeight: 1.6 }}>
      ...
      <div style={{ maxWidth: 420, width: "100%" }}>
        ...
        <div style={{ background: "rgba(255,255,255,0.07)", borderRadius: 24, padding: 28, border: "1px solid rgba(255,255,255,0.12)" }}>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <input value={form.password} onChange={(e) => updateForm("password", e.target.value)} placeholder={isLogin ? "Your password" : "At least 6 characters"} type="password"
          ...
          <p style={{ textAlign: "center", color: "rgba(255,255,255,0.5)", fontSize: 13, margin: "16px 0 0" }}>
            ...
            <button onClick={() => { setMode(isLogin ? "signup" : "login"); setErr(""); }} style={{ background: "none", border: "none", color: B.purpleLight, cursor: "pointer", fontWeight: 700, fontSize: 13 }}>
              {isLogin ? "Sign up" : "Sign in"}
            </button>
          </p>
        </div>
        <button onClick={() => setMode("landing")} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.4)", marginTop: 20, cursor: "pointer", fontSize: 14, display: "block", width: "100%", textAlign: "center" }}>
          ← Back
        </button>
      </div>
    </div>
      ...
      <div style={{ background: B.white, borderRadius: 28, padding: 28, maxWidth: 420, width: "100%", boxShadow: "0 24px 80px rgba(107,33,232,0.3)" }} onClick={(e) => e.stopPropagation()}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          ...
          <div>
            ...
            <p style={{ color: B.inkLight, fontSize: 13, margin: 0 }}>Progress is tracked independently per dialect</p>
          </div>
        </div>
        <p style={{ color: B.inkLight, fontSize: 13, margin: "0 0 16px", background: B.purplePale, borderRadius: 10, padding: "8px 12px" }}>
          💡 Switching won't reset progress — each dialect tracks separately.
        </p>
        {DIALECTS.map((d) => {
    ...
    <div style={{ minHeight: "100vh", background: `linear-gradient(150deg,${B.purpleDark} 0%,#3B0E8C 50%,#0D2040 100%)`, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ maxWidth: 460, width: "100%" }}>
        ...
        {DIALECTS.map((d) => {
          ...
          return (
            <div key={d} onMouseEnter={() => setHov(d)} onMouseLeave={() => setHov(null)} onClick={() => onSelect(d)} style={{ background: hov === d ? "rgba(255,255,255,0.2)" : "rgba(255,255,255,0.1)", border: `2px solid ${hov === d ? B.purpleLight : "rgba(255,255,255,0.15)"}`, borderRadius: 18, padding: "18px 22px", cursor: "pointer", display: "flex", alignItems: "center", gap: 16, marginBottom: 12, transition: "all 0.2s", transform: hov === d ? "scale(1.02)" : "scale(1)" }}>
...
/* ─── HOME ──────────────────────────────────────────────────── */
function Home({ dialect, setScreen, dialectStats, onSwitch, user }) {
    ...
    <div style={{ paddingBottom: 40 }}>
      <div style={{ background: `linear-gradient(135deg,${B.purpleDark} 0%,${B.purpleMid} 70%,#4C1D95 100%)`, padding: "28px 24px 36px", borderRadius: "0 0 36px 36px", marginBottom: 24, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 18 }}>
          ...
          <button onClick={onSwitch} style={{ background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.25)", borderRadius: 12, padding: "8px 14px", color: "#fff", cursor: "pointer", fontSize: 12, fontWeight: 700 }}>Switch ↕</button>
        </div>
      ...
      <div style={{ padding: "0 20px" }}>
        ...
        <Card style={{ marginBottom: 20, padding: 22 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
            ...
            <button onClick={() => speak(wod.ijaw)} style={{ background: B.purplePale, border: "none", borderRadius: 50, width: 40, height: 40, fontSize: 18, cursor: "pointer", color: B.purple, display: "flex", alignItems: "center", justifyContent: "center" }}>🔊</button>
          </div>
...
/* ─── QUIZ ──────────────────────────────────────────────────── */
function Quiz({ lessonContext, setScreen, dialect, dialectStats, updateDialectStats, addXP, isMobile, onSwitch }) {
  const dlessons = getLessons(dialect);
...
/* ─── DICTIONARY ────────────────────────────────────────────── */
function Dictionary({ dialect, isMobile, onSwitch, setScreen }) {
...
/* ─── VOICE ─────────────────────────────────────────────────── */
function Voice({ dialect, isMobile, updateDialectStats, onSwitch, addXP }) {
...
function Progress({ dialect, allStats, isMobile, onSwitch, user, onLogout }) {
  ...
  const totalXP = DIALECTS.reduce((sum, d) => sum + (allStats[d]?.xp || 0), 0);
...
export default function App() {
  ...
  const handleLogout = () => {
    ...
    setAllStats({ Kalabari: freshStats(), Nembe: freshStats(), Izon: freshStats() });
  };

  const handleDialectPick   = (d) => { setDialect(d); setHasChosen(true); };
  const handleDialectSwitch = (d) => {
    setDialect(d); setShowModal(false);
    setScreen("home"); setPlayerCtx(null); setLessonCtx(null);
  };
  ...
  if (screen === "player" && playerCtx) {
    return (
      <LessonPlayer
        context={playerCtx}
        setScreen={(s) => {
          if (s === "quizFromLesson") { setLessonCtx(playerCtx.lesson); setScreen("quiz"); }
          else setScreen(s);
        }}
        addXP={addXP}
      />
  ...
  const shared = {
    dialect, setScreen, isMobile,
    dialectStats, updateDialectStats, allStats,
    addXP,
    onSwitch: () => setShowModal(true),
    user: authUser,
  };
    ...
    <div style={{ display: "flex", fontFamily: "'Segoe UI',system-ui,sans-serif", background: B.off, minHeight: "100vh" }}>
      {!isMobile && (
        <Sidebar
          ...
          dialect={dialect} onSwitch={() => setShowModal(true)}
          stats={dialectStats} user={authUser} onLogout={handleLogout}
        />
      )}
        ...
        {!isMobile && (
          <div style={{ padding: "18px 28px 14px", borderBottom: `1.5px solid ${B.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", background: B.white, position: "sticky", top: 0, zIndex: 100 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div onClick={() => setScreen("progress")} style={{ width: 36, height: 36, borderRadius: 50, background: B.purplePale, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, cursor: "pointer" }}>
                {authUser.avatar}
              </div>
